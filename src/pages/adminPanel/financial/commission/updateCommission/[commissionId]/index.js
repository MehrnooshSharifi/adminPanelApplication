...
const UpdateCommissionItem = ({ commissionInfo, servicesName }) => {
  const router = useRouter();
  const [openIsActive, setOpenIsActive] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const { commissionId } = router.query;
  const initialValues = {
    supperNationalId: commissionInfo?.supperNationalId,
    representativeNationalId: commissionInfo?.representativeNationalId,
    consumerNationalId: commissionInfo?.consumerNationalId,
    serviceId: commissionInfo?.serviceId,
    amount: commissionInfo?.amount,
    isActive: commissionInfo?.isActive,
  };
  const onSubmit = async (values) => {
    const amountValue = values.amount || "";
    const amountWithoutCommas = Number(
      amountValue.toString().replace(/,/g, "")
    );
    setIsLoading(true);
...
export default UpdateCommissionItem;

export async function getServerSideProps(ctx) {
  try {
    const { req, query } = ctx;
    const { commissionId } = query;
    console.log(commissionId);
    const token = req.cookies.Token;
    console.log("token:", token);
    const { data } = await SelectCommissionById(commissionId, token);
    const info = await FindServiceName(token);
    console.log(data);
    return {
      props: { commissionInfo: data, servicesName: info.data },
    };
  } catch (error) {
