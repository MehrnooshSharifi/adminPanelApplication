...
const BlackListTable = ({ blockedList }) => {
  ...
  const currentItems = blockedList?.slice(indexOfFirstItem, indexOfLastItem);
  const [selectedId, setSelectedId] = useState(null);
  const [isOpen, setIsOpen] = useState(true);
  const openHandler = (id) => {
    for (let element of blockedList) {
  ...
  return (
    <div className="flex flex-col gap-y-[70px] -mt-[20px] ">
      <div className=" flex flex-col max-w-[340px] md:max-w-[680px] lg:max-w-[1440px] overflow-x-auto overflow-y-hidden ">
        ...
        <div className="flex min-w-[900px]   lg:w-[1030px] h-[60px] bg-neutralColor-5 pt-[6px] ">
          <div className="flex text-[12px] md:text-[16px]  font-medium  text-neutralColor-2 items-center whitespace-nowrap rounded-[6px] h-[43px]">
            ...
            <div className=" mr-[80px] md:mr-[80px] ">
              <span className="w-[103px] h-[28px] lg:-mr-[10px]">کد/شناسه ملی</span>
            </div>
            <div className=" mr-[80px] md:mr-[80px] lg:mr-[90px]">
              <span className="w-[103px] h-[28px]">تاریخ ورود به لیست</span>
            </div>
            <div className=" mr-[90px] md:mr-[70px] lg:mr-[60px] ">
              <span className="w-[103px] h-[28px] lg:mr-[20px] ">
                تاریخ خروج از لیست
              </span>
            </div>
            <div className=" mr-[80px] md:mr-[60px] lg:mr-[50px]">
              <span className="w-[103px] h-[28px] lg:mr-[20px]">
                کدملی بلاک کننده
              </span>
            </div>
        ...
        {/* body */}
        {currentItems?.map((blockedItem, index) => {
          return (
            <div key={blockedItem.nationalCode} className="">
              {/* GroupServices */}
              <div
              ...
              >
                <div
                  className={`w-[95px] h-[37px] md:w-[106px]  md:h-[45px] ml-[60px] md:ml-[105px] mr-[10px] py-2 rounded-[50px] flex items-center justify-center  ${
                    !blockedItem.isBlocked
                      ? "bg-errorColor-5 text-errorColor-2"
                      : "bg-successColor-5 text-successColor-2"
                  }`}
                >
                  <span>{blockedItem.isBlocked ? "فعال" : "غیرفعال"}</span>
                </div>
                <div className=" w-[100px] h-[57px] flex items-center  -mr-[5px] md:-mr-[80px] ">
                  <span className="mr-[5px] md:mr-[30px] lg:mr-[15px]">
                    {blockedItem.nationalCode}
                  </span>
                </div>
                <div className=" w-[150px] h-[57px] flex items-center  mr-[45px] md:mr-[80px] cursor-pointer ">
                  <span className="lg:truncate whitespace-nowrap mr-[10px] md:mr-[20px] lg:mr-[20px]">
                    {blockedItem.startBlockDate}
                  </span>
                </div>
                <div className=" w-[100px] h-[57px] flex items-center whitespace-nowrap mr-[50px] md:mr-[50px] ">
                  <span className="md:mr-[20px] lg:mr-[30px] ">
                    {blockedItem.endBlockDate}
                  </span>
                </div>
                <div className=" h-[57px] flex items-center max-w-[200px] mr-[60px]  md:mr-[30px] ">
                  <span className="max-w-[500px] mr-[10px] md:mr-[80px] lg:mr-[100px] ">
                    {blockedItem.blockerAdminNationalCode}
                  </span>
                </div>
              </div>
              {/* operations section */}
              ...
              >
                {!isOpen && blockedItem.nationalCode == selectedId ? (
                  <div className="flex items-baseline relative">
                    <div className="flex flex-col w-[100px] h-[48px] md:w-[115px] md:h-[51px]  rounded-[10px] border text-[12px] md:text-[14px] text-neutralColor-1 font-medium whitespace-nowrap  z-30   border-neutralColor-4 bg-naturalColor-2 fixed mr-[10px] md:mr-[120px] shadow-lg">
                      ...
                      <Link
                        ...
                        href={`/adminPanel/blackList/updateBlackListItem/${blockedItem.nationalCode}`}
                      >
                        <div>
                    ...
                    >
                      <Image
                        onClick={() => openHandler(blockedItem.nationalCode)}
                        className="mb-1"
      ...
      <div className="z-10  -mt-[100px]">
        {blockedList && blockedList.length > 5 && (
          <Pagination
            siblingCount={0}
            color="primary"
            size="medium"
            count={Math.ceil(blockedList.length / itemsPerPage)}
            page={currentPage}
