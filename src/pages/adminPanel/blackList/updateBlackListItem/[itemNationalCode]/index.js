...
import { GetAllBlockedList, UpdateBlockUser } from "@/src/server/Service";
...
const UpdateBlackListItem = ({ allBlockedList }) => {
  ...
  const { itemNationalCode } = router.query;

  const blockedUserInfo = allBlockedList.filter((item) => {
    return item.nationalCode === itemNationalCode;
  });

  const initialValues = {
    nationalCode: blockedUserInfo[0]?.nationalCode,
    isBlocked: blockedUserInfo[0]?.isBlocked,
  ...
  const onSubmit = async (values) => {
    setIsLoading(true);
    const res = await UpdateBlockUser(values, dateObject);
...
export default UpdateBlackListItem;
...
export async function getServerSideProps(ctx) {
  try {
    ...
    const { data } = await GetAllBlockedList(token);
    console.log(data);
    return {
      props: { allBlockedList: data },
    };
  } catch (error) {
