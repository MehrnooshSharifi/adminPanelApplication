import Link from "next/link";
import { Pagination, PaginationItem, Tooltip } from "@mui/material";
import ArrowBackIosIcon from "@mui/icons-material/ArrowBackIos";
import ArrowForwardIosIcon from "@mui/icons-material/ArrowForwardIos";
import { useState } from "react";
const DesktopKeysTable = ({ allScope }) => {
  const [currentPage, setCurrentPage] = useState(1);
  const [itemsPerPage, setItemsPerPage] = useState(5);
  const handleChangePage = (event, newPage) => {
    setCurrentPage(newPage);
  };
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = allScope.slice(indexOfFirstItem, indexOfLastItem);
  return (
    <div className="flex flex-col gap-y-[70px] items-center">
      <div className=" hidden lg:flex flex-col  max-w-[1030px]  overflow-y-clip items-center ">
        {/* header */}
        <div className=" bg-neutralColor-5 flex h-[45px] rounded-[6px] w-[1030px] ">
          <div className="flex items-center text-[16px] font-medium leading-[27.64px] text-neutralColor-2 w-fit mr-[17px] gap-x-[70px]">
            <div className="w-[40px]">
              <span className="mr-4">وضعیت</span>
            </div>
            <div className="w-[75px] whitespace-nowrap">
              <span className="mr-[60px]">تعداد دستگاه ها</span>
            </div>
            <div className="w-[52px] whitespace-nowrap">
              <span className="mr-[130px]">تاریخ انقضاء</span>
            </div>
            <div className="w-[52px] whitespace-nowrap">
              <span className="mr-[210px]">Key</span>
            </div>
            <div className="w-[100px]  whitespace-nowrap">
              <span className="mr-[265px]">Scope</span>
            </div>
          </div>
        </div>
        {/* body */}
        {currentItems.map((scope, index) => {
          return (
            <div key={scope.id} className="flex">
              <div
                className={` px-[20px]  flex w-[1030px] h-[91px] items-center py-[23px]  text-[14px] text-neutralColor-1 leading-[27.64px] gap-x-[70px] ${
                  index % 2 === 0
                    ? "bg-naturalColor-2"
                    : "bg-neutralColor-5 border-t border-t-neutralColor-4 border-b border-b-neutralColor-4"
                }`}
              >
                <div
                  className={`w-[90px] h-[40px]  flex items-center justify-center rounded-[100px] ml-[70px] cursor-pointer ${
                    scope.isActive
                      ? "bg-successColor-5 text-successColor-2"
                      : "bg-errorColor-5 text-errorColor-2"
                  }`}
                >
                  <span>{scope.isActive ? "فعال" : "غیرفعال"}</span>
                </div>
                <div className="w-[99px]">
                  <span className="-mr-[15px]">{scope.deviceCount}</span>
                </div>
                <div className="w-[99px] whitespace-nowrap">
                  <span className="-mr-[40px]">{scope.expireDate}</span>
                </div>
                <div className="w-[99px]">
                  <span className="mr-[20px]">{scope.pfxKey}</span>
                </div>
                <div className="w-[89px]">
                  <span className="mr-[30px]">{scope.scopeId}</span>
                </div>
              </div>
              <div className="flex  items-center fixed mr-[900px] mt-[20px]">
                <Link
                  className=" bg-primaryColor-1 text-naturalColor-2 whitespace-nowrap border  text-[16px] font-medium leading-[27.64px] py-[8px] px-[16px] rounded-[5px] w-[110px] h-[44px]  flex justify-center items-center"
                  href={`/adminPanel/userManagement/newRequests/newRequestDetails/${scope.scopeId}`}
                >
                  <span>ویرایش</span>
                </Link>
              </div>
            </div>
          );
        })}
      </div>
      <div className="z-10 hidden lg:block overflow-y-clip -mt-[80px]">
        {allScope && allScope.length > 5 && (
          <Pagination
            siblingCount={0}
            color="primary"
            size="medium"
            count={Math.ceil(allScope.length / itemsPerPage)}
            page={currentPage}
            onChange={handleChangePage}
            shape="rounded"
            renderItem={(item) => (
              <PaginationItem
                slots={{
                  previous: ArrowForwardIosIcon,
                  next: ArrowBackIosIcon,
                }}
                {...item}
              />
            )}
          />
        )}
      </div>
    </div>
  );
};

export default DesktopKeysTable;
