...
import SupervisorInfo from "@/src/components/supervisorList/SupervisorInfo";
// import SupervisorInfo from "@/src/components/supervisorList/SupervisorInfo";
import SupervisorTable from "@/src/components/supervisorList/SupervisorTable";
import {
  GetAllAdmin,
...
const SupervisorUser = ({ adminUsers }) => {
  ...
  const onSubmit = async (values) => {
    ...
    if (values.nationalCode !== "") {
      ...
      if (res.isSuccess && res.data.length < 1) {
  ...
  return (
    <>
      <SearchForm formik={formik} title="لیست کاربران ادمین">
        <button
          type="submit"
      ...
      >
        ...
        <div className="felx flex-col mt-[22px] md:mt-[34.88px] lg:mt-[35px]  lg:mx-auto">
          ...
          <div className="lg:mr-[50px] flex relative">
            ...
            <Link
              href="/adminPanel/supervisorUsers/addToSupervisorList"
              className={`${
                adminUsers.length == 0
            ...
            <div>
              {adminUsers && adminUsers.length !== 0 && !user && (
                <SupervisorTable adminUsers={adminUsers} />
              )}
            </div>
            <div>
              {user && user.length !== 0 && (
                <SupervisorInfo adminUser={user[0]} />
              )}
            </div>
          </div>
        </div>
