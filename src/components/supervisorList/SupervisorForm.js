        ...
        >
          ...
          <div className={`${isOpen ? "block" : "hidden"}`}>
            {roles.map((role) => {
              return (
                <div
                  key={role.publicAppId}
                  className="h-[40px] lg:h-[50px]  flex items-center gap-x-[12px] px-4 border-b border-b-neutralColor-4 relative "
                >
                  <Checkbox
                    checked={role.isActivePublicApp}
                    name={`${role.publicAppId}`}
                    type="checkbox"
                    value={role.publicAppId}
                    onClick={() => changeRoleHandler(role.publicAppId)}
                  />
                  <label>
                    {role.publicAppId == "46801160-75c5-4486-a7c5-09ddfe0e0375"
                      ? "سرپرست کل"
                      : role.publicAppId ==
                        "284c7daf-53e2-49d5-8978-2814d714e3c3"
                      ? "مالی"
                      : "پشتیبانی"}
                  </label>
                </div>
