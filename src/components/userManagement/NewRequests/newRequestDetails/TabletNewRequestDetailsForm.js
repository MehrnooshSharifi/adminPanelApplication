import Image from "next/image";
import { useState } from "react";
import { useRef } from "react";
import AcceptNewRequestModal from "./checkNewRequest/AcceptNewRequestModal";
import RejectNewRequestModal from "./checkNewRequest/RejectNewRequestModal";

const TabletNewRequestDetailsForm = ({ formik }) => {
  const [showRejectRequestModal, setShowRejectRequestModal] = useState(false);
  const [showAcceptRequestModal, setShowAcceptRequestModal] = useState(false);
  const defaultPrice = useRef();
  const focusDefaultPrice = () => {
    defaultPrice.current.focus();
  };
  const rejectRequestHandler = (e) => {
    e.preventDefault();
    setShowRejectRequestModal(true);
  };
  const acceptRequestHandler = (e) => {
    e.preventDefault();
    setShowAcceptRequestModal(true);
  };
  return (
    <div className="flex items-center justify-center">
      <form className="md:flex flex-col gap-y-[36px] hidden pr-5 lg:pr-[100px] relative ">
        {/* serviceGroupName&serviceName */}
        <div className="flex gap-x-[25px] lg:gap-x-[40px]">
          {/* serviceGroupName */}
          <div className="flex flex-col relative">
            <label
              className=" md:w-[125px] md:h-[20px] lg:w-[140px] lg:h-[20px] absolute -top-3  [5px] px-[10px] whitespace-nowrap mr-2 bg-naturalColor-2 flex flex-col text-[14px] lg:text-[16px] lg:-top-4 font-bold text-neutralColor-3"
              htmlFor="serviceGroupName"
            >
              <span className="">نام گروه سرویس</span>
            </label>
            <input
              readOnly
              autoComplete="off"
              className="w-[327px] lg:w-[396px] lg:h-[48px] h-[50px] text-[14px] bg-neutralColor-5 text-neutralColor-3 rounded-[5px] border border-neutralColor-3 py-[12px] pr-[16px]"
              id="serviceGroupName"
              name="serviceGroupName"
              type="text"
              value={formik.values.serviceGroupName}
            />
          </div>
          {/* serviceName */}
          <div className="flex flex-col relative max-w-[327px] ">
            <label
              className="flex flex-col px-[10px] absolute -top-3 text-[14px] lg:text-[16px] lg:-top-4  mr-2 font-bold text-neutralColor-3 bg-naturalColor-2"
              htmlFor="serviceName"
            >
              <span>نام سرویس</span>
            </label>
            <input
              readOnly
              className="w-[327px] h-[50px] lg:w-[396px] lg:h-[48px]  text-[14px] leading-24.18px] bg-neutralColor-5 text-neutralColor-3 font-medium rounded-[5px] border border-neutralColor-3 py-[12px] pr-[16px] "
              name="serviceName"
              value={formik.values.serviceName}
            />
          </div>
        </div>
        {/* appName&serviceName */}
        <div className="flex gap-x-[25px] lg:gap-x-[40px]">
          {/*appName */}
          <div className="flex flex-col relative">
            <label
              className=" md:w-[100px] md:h-[20px] lg:w-[110px] lg:h-[20px] absolute -top-3  [5px] px-[10px] whitespace-nowrap mr-2 bg-naturalColor-2 flex flex-col text-[14px] lg:text-[16px] lg:-top-4 font-bold text-neutralColor-3"
              htmlFor="appName"
            >
              <span className="">نام اپلیکیشن</span>
            </label>
            <input
              readOnly
              autoComplete="off"
              className="w-[327px] lg:w-[396px] lg:h-[48px] h-[50px] text-[14px] bg-neutralColor-5 text-neutralColor-3 rounded-[5px] border border-neutralColor-3 py-[12px] pr-[16px]"
              id="appName"
              name="appName"
              type="text"
              value={formik.values.appName}
            />
          </div>
          <div className="flex flex-col relative">
            <label
              className=" md:w-[60px] md:h-[20px] lg:w-[70px] lg:h-[20px] absolute -top-3  [5px] px-[10px] whitespace-nowrap mr-2 bg-naturalColor-2 flex flex-col text-[14px] lg:text-[16px] lg:-top-4 font-bold text-neutralColor-3"
              htmlFor="appName"
            >
              <span className="">Scope</span>
            </label>
            <input
              readOnly
              autoComplete="off"
              className="w-[327px] lg:w-[396px] lg:h-[48px] h-[50px] text-[14px] bg-neutralColor-5 text-neutralColor-3 rounded-[5px] border border-neutralColor-3 py-[12px] pr-[16px]"
              id="appName"
              name="appName"
              type="text"
              value={formik.values.publicAppId}
            />
          </div>
        </div>
        {/* defaultPrice */}
        <div className="flex flex-col relative">
          <label
            className=" md:w-[90px] md:h-[18px] lg:w-[100px] lg:h-[20px] absolute -top-3  [5px] px-[10px] whitespace-nowrap mr-2 bg-naturalColor-2 flex flex-col text-[14px] lg:text-[16px] lg:-top-4 font-bold text-neutralColor-2"
            htmlFor="defaultPrice"
          >
            <span className="">مبلغ (ریال)</span>
          </label>
          <input
            ref={defaultPrice}
            autoComplete="off"
            className="w-[327px] lg:w-[396px] lg:h-[48px] h-[50px] text-[14px] text-neutralColor-1 rounded-[5px] border border-neutralColor-3 py-[12px] pr-[16px]"
            id="defaultPrice"
            name="defaultPrice"
            type="text"
            value={formik.values.defaultPrice}
            onChange={formik.handleChange}
          />
          <div
            className="w-fit absolute top-[15px] right-[290px] lg:right-[360px] lg:top-[12px] cursor-pointer"
            onClick={focusDefaultPrice}
          >
            <Image
              alt="edit"
              src="/assets/images/yellowEdit.svg"
              width={20}
              height={21}
            />
          </div>
        </div>
        {/* requestId */}
        <div className="hidden">
          <div className="flex flex-col relative">
            <label
              className=" md:w-[100px] md:h-[20px] lg:w-[110px] lg:h-[20px] absolute -top-3  [5px] px-[10px] whitespace-nowrap mr-2 bg-naturalColor-2 flex flex-col text-[14px] lg:text-[16px] lg:-top-4 font-bold text-neutralColor-3"
              htmlFor="appName"
            >
              <span className="">RequestId</span>
            </label>
            <input
              readOnly
              autoComplete="off"
              className="w-[327px] lg:w-[396px] lg:h-[48px] h-[50px] text-[14px] bg-neutralColor-5 text-neutralColor-3 rounded-[5px] border border-neutralColor-3 py-[12px] pr-[16px]"
              id="appName"
              name="appName"
              type="text"
              value={formik.values.id}
            />
          </div>
        </div>
        {/* submit */}
        <div className="flex flex-col gap-y-[20px] mt-[250px] lg:flex-row relative ">
          <button
            onClick={acceptRequestHandler}
            type="button"
            className="bg-primaryColor-1  lg:-right-[100px]  rounded-[5px] text-center lg:absolute  lg:-top-[5px]   px-[16px] text-naturalColor-2 text-[16px] font-medium  w-[680px] h-[48px] lg:w-[157px] lg:h-[44px] lg:text-[16px] mt-[80px]"
          >
            تایید درخواست
          </button>
          <button
            onClick={rejectRequestHandler}
            type="button"
            className="bg-naturalColor-2  lg:top-[80px] lg:right-[700px]  rounded-[5px] text-center lg:absolute border border-primaryColor-1  px-[16px] text-primaryColor-1 text-[16px] font-medium  w-[680px] h-[48px] lg:w-[157px] lg:h-[44px] lg:text-[16px]"
          >
            رد درخواست
          </button>
        </div>
      </form>
      {showAcceptRequestModal && (
        <AcceptNewRequestModal
          setShowAcceptRequestModal={setShowAcceptRequestModal}
          formik={formik}
        />
      )}
      {showRejectRequestModal && (
        <RejectNewRequestModal
          formik={formik}
          setShowRejectRequestModal={setShowRejectRequestModal}
        />
      )}
    </div>
  );
};

export default TabletNewRequestDetailsForm;
