...
const DesktopUserServicesTable = ({ userServices }) => {
  ...
  const columns = [
    {
      ...
      render: (approveStates) => (
        <div
          className={`w-[90px] h-[40px] flex items-center justify-center rounded-[100px] cursor-pointer ${
            approveStates == 1
      ...
      sorter: (a, b) => a.approveStates - b.approveStates,
    },
    {
      title: "سرویس",
    ...
    //       <span className="truncate cursor-pointer">{appName}</span>
    //     </Tooltip>
    //   ),
  ...
  return (
    <div className="hidden lg:flex flex-col items-center max-w-[1030px] mx-auto h-auto">
      {userServices && userServices.length > 0 ? (
        <Table
          dataSource={userServices.map((service) => ({
            key: service.id, // Ensure `key` is service.id
            id: service.id, // Map `id` correctly
...
export default DesktopUserServicesTable;
